SMODS.Joker{ --Flintlock
    ...
    calculate = function(self, card, context)
        if context.cardarea == G.jokers and context.joker_main  then
            if true then
                if SMODS.pseudorandom_probability(card, 'group_0_2a8a55f3', 1, card.ability.extra.odds, 'j_sauce_flintlock') then
                      SMODS.calculate_effect({Xmult = card.ability.extra.Xmult}, card)
                  end
            elseif true then
                if SMODS.pseudorandom_probability(card, 'group_0_d42ebed3', 1, card.ability.extra.odds, 'j_sauce_flintlock') then
